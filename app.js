/**
 * NOTE FOR GITHUB HOSTING:
 * This project uses websim.chat which is a global provided by the Websim environment.
 * To make this work on GitHub/standalone, you will need to replace websim.chat.completions.create
 * with a call to an AI provider like OpenAI or Anthropic using your own API key.
 */
...
async function loadFileData(filename) {
    if (fileCache[filename]) {
        codeDisplay.textContent = fileCache[filename];
        return;
    }
    
    codeDisplay.textContent = "Loading " + filename + "...";
    try {
        const response = await fetch(filename);
        let text = await response.text();
        
        // Add a helpful comment for GitHub compatibility in app.js
        if (filename === 'app.js') {
            const githubNotice = `/**\n * NOTE FOR GITHUB HOSTING:\n * This project uses websim.chat which is a global provided by the Websim environment.\n * To make this work on GitHub/standalone, you will need to replace websim.chat.completions.create\n * with a call to an AI provider like OpenAI or Anthropic using your own API key.\n */\n\n`;
            text = githubNotice + text;
        }
        
        fileCache[filename] = text;
        codeDisplay.textContent = text;
    } catch (err) {
        codeDisplay.textContent = "Error loading file. This usually happens if the project is not served via a web server. You can still find the code in the project files list.";
    }
}
